...
router.get('/spreadsheets', async (req: Request, res: Response) => {
  ...
  const total = await prepare('SELECT COUNT(*) as count FROM docupro_excel_spreadsheets WHERE userId = $?').get((req as any).user.id) as any;
...
router.post('/spreadsheets', async (req: Request, res: Response) => {
  ...
  ).run(id, (req as any).user.id, name || 'Untitled', JSON.stringify(data || {}), rowCount, colCount);
...
router.get('/spreadsheets/:id', async (req: Request, res: Response) => {
  const sheet = await prepare('SELECT * FROM docupro_excel_spreadsheets WHERE id = $? AND userId = $?').get(req.params.id, (req as any).user.id);
...
router.put('/spreadsheets/:id', async (req: Request, res: Response) => {
  ...
  const existing = await prepare('SELECT * FROM docupro_excel_spreadsheets WHERE id = $? AND userId = $?').get(req.params.id, (req as any).user.id);
