...
router.get('/documents', async (req: Request, res: Response) => {
  ...
  const total = await prepare('SELECT COUNT(*) as count FROM docupro_word_documents WHERE userId = $?').get((req as any).user.userId) as any;
...
router.post('/documents', async (req: Request, res: Response) => {
  ...
  ).run(id, (req as any).user.userId, name || 'Untitled', content, format, 0);
  const doc = await prepare('SELECT * FROM docupro_word_documents WHERE id = $?').get(id);
  res.status(201).json(doc);
});

router.get('/documents/:id', async (req: Request, res: Response) => {
  const doc = await prepare('SELECT * FROM docupro_word_documents WHERE id = $? AND userId = $?').get(req.params.id, (req as any).user.userId);
...
router.put('/documents/:id', async (req: Request, res: Response) => {
  ...
  const existing = await prepare('SELECT * FROM docupro_word_documents WHERE id = $? AND userId = $?').get(req.params.id, (req as any).user.userId);
